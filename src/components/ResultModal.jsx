...
export default function ResultModal({ open, winner, onClose, onSpinAgain }) {
  useEffect(() => {
    if (!open) return;
    const handler = (e) => {
      if (e.key === 'Escape') {
      ...
      >
        ...
        <div className="mt-4 flex justify-end gap-3">
          ...
          <button
            type="button"
            onClick={onSpinAgain}
            className="inline-flex items-center justify-center rounded-full bg-emerald-500 px-4 py-1.5 text-xs font-semibold text-slate-900 shadow-sm shadow-emerald-500/40 hover:bg-emerald-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900"
          >
            Spin Again
          </button>
        </div>
